        ...
        for (const review of rawReviews) {
            ...
            try {
                if (customer && customer.avatar) {
                    const parsedAvatar = JSON.parse(customer.avatar);
                    avatar = Array.isArray(parsedAvatar) ? parsedAvatar[0] : parsedAvatar;
                }
            }
            catch (e) {
                avatar = customer.avatar; // Nếu không phải JSON thì lấy luôn
            }
            reviews.push({
                ...review,
                customerName: customer ? customer.fullName : "Khách ẩn danh",
